...
#import "CMDingqiCell.h"

@implementation CMDingqiCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [self addSubview:self.dingJoinBtn];
        
        [self.dingJoinBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        [self addSubview:self.dingqiTitle];

        [self.dingqiTitle mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.mas_left).offset(15);
        ...
        [joinPeople mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(self.dingqiTitle);
            make.right.equalTo(self.mas_right).offset(-15);
       ...
       // peopleImage.frame=CGRectMake(225, 20, 12, 12);
        ...
        KDGoalBar *globe=[[KDGoalBar alloc]initWithFrame:CGRectMake(CMScreenW-15-self.dingJoinBtn.bounds.size.width-54,self.bounds.size.height/2.0+15, 54, 54)];
        globe.backgroundColor=[UIColor whiteColor];
        self.dingPercentView=globe;
       ...
       // shouYiZhi.frame=CGRectMake(2 , 45, 64, 58);
        ...
        [shouYiZhi mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.dingJoinBtn);
       ...
       // shouYiZhiXiao.frame=CGRectMake(68 , 51, 57, 28);
        self.dingXiaoshu=shouYiZhiXiao;
       ...
       // shouyi.frame=CGRectMake(72 , 76, 47, 20);
        ...
        [qiXian mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(self.dingJoinBtn.mas_centerX).offset(25);
...
#pragma mark Lazy
-(UIButton*)dingJoinBtn{
    if (!_dingJoinBtn) {

        _dingJoinBtn=[UIButton buttonWithType:UIButtonTypeCustom];
        //Join.frame=CGRectMake(11, 112, 300, 30);
        [_dingJoinBtn setTitle:@"参与" forState:UIControlStateNormal];
        _dingJoinBtn.titleLabel.font=[UIFont systemFontOfSize:18];
        [_dingJoinBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
      
        _dingJoinBtn.backgroundColor=RedButtonColor;
        _dingJoinBtn.layer.cornerRadius=5.0;
        _dingJoinBtn.clipsToBounds=YES;
    }
    return _dingJoinBtn;
}
...
-(UILabel*)dingqiTitle{
    if (!_dingqiTitle) {
       _dingqiTitle=[[UILabel alloc]init];
      _dingqiTitle.font=[UIFont systemFontOfSize:13.0];
