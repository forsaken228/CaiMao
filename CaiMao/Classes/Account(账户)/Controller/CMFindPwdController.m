...
#import "CMFindPwdController.h"

@interface CMFindPwdController ()<UITextFieldDelegate>
...
@implementation CMFindPwdController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"重置密码";
    [self createForgetUiView];
    [self setupUI];
}
...
- (void)getCodeBtnClick
{
    ...
    NSString *urlAuthStr = [NSString stringWithFormat:@"%@/handler/app_interface.ashx?action=SendVerCode&mobile=%@",OnLineCode,self.phoneTextField.text];

    [CMHttpTool getWithURL:urlAuthStr params:nil success:^(id responseObj) {
        
        DLog(@"FindgetCodeBtnClick---%@",responseObj);
...
//        DLog(@"FindPwdCodeshibai------");
    }];
}

- (void)finishBtnClick
{
    DLog(@"--%@",[self.pwdTextField.text substringToIndex:1]);
    ...
    if (![self.pwdTextField.text isEqualToString:self.quePwdTextField.text]) {
        ...
        NSString *regUrlStr = [NSString stringWithFormat:@"%@/handler/AppInterface.ashx?action=ChangePass&mobile=%@&pwd=%@&vercode=%@",OnLineCode,self.phoneTextField.text,self.pwdTextField.text,self.codeTextField.text];
        ...
        [CMHttpTool getWithURL:regUrlStr params:nil success:^(id responseObj)
         {
             ...
             if ([statusStr isEqualToString:@"200"]) {
                 UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"财猫网提示" message:@"重置密码成功！" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                 [alert show];
             }
             if ([statusStr isEqualToString:@"400"]) {
                 UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"财猫网提示" message:resultStr delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
...
- (void)textFieldChange:(CMTextField *)textField
{
    ...
    if (([_phoneTextField.text length] > 0 && [_pwdTextField.text length] > 0) && ([_codeTextField.text length] > 0 && [_quePwdTextField.text length] > 0)) {
        ...
        [self.finishBtn setBackgroundColor:RedButtonColor];
    } else {
...
#pragma mark 请确认密码
    ...
    [CommtionBtn setTitle:@"完成" forState:UIControlStateNormal];
    [CommtionBtn addTarget:self action:@selector(finishBtnClick) forControlEvents:UIControlEventTouchUpInside];
